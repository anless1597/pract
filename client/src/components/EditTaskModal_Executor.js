...
const EditTaskModal_Executor = observer(() => {
    ...
    function closeTask(e) {
        let modalContent = (document.getElementById('task-edit-executor')).getElementsByClassName('modal-content')[0];
        if (e.target.closest('.' + modalContent.className) === null) {
            (document.getElementById('task-edit-executor')).classList.remove('modal-active');
        }
    }
    ...
    return (
        <div className="modal" id="task-edit-executor" onMouseDown={closeTask}>
            <div className="modal-content">
                <div className="header">Редактирование задачи</div>
                <div className="modal-input">
                    <label htmlFor="new-task-header">Заголовок </label>
                    <input readOnly type="text" id="new-task-header" value={head} onChange={e=>setHead(e.target.value)} />
                </div>
                <div className="modal-input">
                    <label htmlFor="new-task-description">Описание</label>
                    <p><textarea readOnly id="new-task-description" cols="45" rows="5" value={description} onChange={e=>setDescription(e.target.value)}></textarea></p>
                </div>
                <div className="modal-input">
                    <label htmlFor="new-task-end-date">Дата окончания</label>
                    <input readOnly type="date" id="new-task-end-date" value={endDate} onChange={e=>setEndDate(e.target.value)}/>
                </div>
                <div className="modal-input">
                    <label htmlFor="new-task-priority">Приоритет</label>
                    <select disabled id="new-task-priority" value={tasks.selectedPriority} onChange={(e)=> {
                ...
                <div className="modal-input">
                    ...
                    <select disabled id="new-task-executor" value={tasks.selectedExecutor} onChange={(e)=> {
...
export default EditTaskModal_Executor